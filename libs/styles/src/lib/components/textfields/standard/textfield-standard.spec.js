  ...
  [
    ...
    },
    {
      ...
      states: {
        '#xmas': {
          style:
            '--uiseek-primary-rgb: 255, 0, 0;--uiseek-onsurface-rgb: 255, 255, 255;width: 240px;',
        },
        '#xmas > input': ['focus'],
      },
      bodyColor: { a: 0 },
    ...
    describe(`in ${suite.label} state`, () => {
      ...
      beforeAll(async () => {
        style = setUp(
          'src/components/textfields/standard/textfield-standard',
          suite.states
        );
    ...
    describe(`(textarea) in ${suite.label} state`, () => {
      ...
      beforeAll(async () => {
        // Select textarea
        const states = Object.entries(suite.states).reduce(
          (object, [key, value]) => {
            const newKey = key
              .replace(/#xmas/g, '#ta-xmas')
              .replace(/input/g, 'textarea');
            object[newKey] = value;
            if (key === '#xmas') {
