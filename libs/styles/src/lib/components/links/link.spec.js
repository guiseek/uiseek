...
describe('Link', () => {
  [
    ...
    },
    {
      ...
      states: {
        '#xmas.uiseek-link': ['hover', 'focus', 'active'],
      },
      textColor: { r: 33, g: 150, b: 243, a: 255 },
      bodyColor: tp,
      underline: true,
    },
    {
      ...
      states: {
        '#xmas.uiseek-link': {
          style:
            '--uiseek-primary-rgb: 255, 0, 0;display: inline-block; width: 80px; font-family: "Roboto", "Segoe UI", BlinkMacSystemFont, system-ui, -apple-system; font-size: 16px;',
        },
      },
    ...
    describe(`in ${suite.label} state`, () => {
      ...
      it(`should${suite.underline ? '' : ' not'} have underline`, () => {
        ...
        isBrowserNot('Firefox') &&
          expect(underline).toResembleColor(
            suite.underline ? suite.textColor : suite.bodyColor
          );
        isBrowser('Firefox') &&
          expect(underlineFF).toResembleColor(
            suite.underline ? suite.textColor : suite.bodyColor
          );
      });
    });
