...
@Directive({ selector: 'input[uiSeek][currency]' })
export class UiSeekCurrencyMaskDirective implements OnDestroy, AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    if (!this.control) {
      console.warn(
        'UiSeekCurrencyMask: Um FormControl é necessário para que a diretiva seja iniciada.'
      )
      return
    }
    ...
    if (!this.nativeEl) {
      console.warn(
        'UiSeekCurrencyMask: um elRef do tipo input é necessário para que a diretiva seja iniciada.'
      )
      return
    }
