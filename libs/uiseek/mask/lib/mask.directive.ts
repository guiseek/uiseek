...
@Directive({
  selector: 'input[uiSeek][mask]',
})
export class UiSeekMaskDirective implements OnDestroy, OnInit {
  ...
  ngOnInit() {
    if (!this.mask) {
      console.warn(
        'UiSeekMask: Um valor de Máscara é necessário para que a diretiva seja iniciada.'
      )
      return
    }
    ...
    if (!this.control) {
      console.warn(
        'UiSeekMask: É necessário um FormControl para que a diretiva seja iniciada.'
      )
      return
    }
    ...
    if (!this.nativeEl) {
      console.warn(
        'UiSeekMask: Um elRef do tipo de entrada é necessário para que a diretiva seja iniciada.'
      )
      return
    }
