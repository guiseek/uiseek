    ...
    webSocketServer.on('request', (request) => {
      ...
      connection.on('message', (message) => {
        ...
        if (message.type === 'utf8') {
          ...
          if (messageObject.typeName === 'Message' && this.connectionIdMap.has(connection)) {

            const connectionSourceId = this.connectionIdMap.get(connection);
            const { sourceId, targetId, contentType, contentJson } = messageObject as Message

            const isValidMessage = connectionSourceId
              && sourceId === sourceId
              && targetId && contentType && contentJson
            ...
            if (isValidMessage) {
              ...
              if (this.idConnectionMap.has(targetId)) {
                ...
                console.log('signaling server relayed message ' + message.join(' '));
